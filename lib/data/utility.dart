...
int totalForSpecificTransaction(String transactionType) {
  var history2 = box.values.toList();
  List<int> amounts = [];

  for (var i = 0; i < history2.length; i++) {
    if (history2[i].name.toLowerCase() == transactionType.toLowerCase() &&
        history2[i].IN.toLowerCase() == 'expense') {
      amounts.add(int.parse(history2[i].amount));
    }
  }

  if (amounts.isEmpty) {
    return 0; // Return 0 if no transactions found
  }
